...
Component.prototype.relay = function( name, data, onResult ){
  ...
  function reactTo( receivers ){
    ...
    var i = -1
      , l = receivers.length
    while( ++i < l ){
      receivers[i](relayIntent, onResult)
  ...
  while( ++i < l ){
    ...
    // handle local root
    reactTo(component._receivers[name])
...
Component.prototype.reactTo = function( name, receiver ){
  if( this._receivers[name] ){
    this._receivers[name].push(receiver)
  }
  else {
